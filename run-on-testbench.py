...
def extract_variables(file_name):
    pattern = r'sample-v(\d+)-e(\d+)-ld(\d+)-wd(\d+)-try(\d+)\.bc'
    ...
    tries = int(match.group(5))
    return vertices, edges, length_depth, width_depth, tries
...
output_file.write('test_file,vertices,edges,length_depth,width_depth,tries,time\n')

periodic_flush = 0

bc_files = os.listdir(options.target_dir)    
for bc_file in bc_files:
    vertices, edges, length_depth, width_depth, try_no = extract_variables(bc_file)
    ...
    output_file.write(f"{bc_file},{vertices},{edges},{length_depth},{width_depth},{try_no},{exec_time}\n")
